...
export interface ReverbParams {
  key: string; // For tap structures
  size: number; // Reverb size/length
  decay: number; // Reverb decay time
  mod: number; // Modulation depth
}
...
export function diffuse(size: any, ...ins: any[]) {
  ...
  const dels = ins.map(function (input: any, i: number) {
    const lineSize = size * ((i + 1) / len);
    return el.sdelay({ size: lineSize }, input);
  });

  return H8.map(function (row: any) {
    return el.add(
      ...row.map(function (col: any, j: number) {
...
export function dampFDN(name: string, sampleRate: number, size: any, decay: any, modDepth: any, ...ins: any[]) {
  const len = ins.length;
  const scale = Math.sqrt(1 / len);
  ...
  const dels = ins.map(function (input: any, i: number) {
    return el.add(input, el.mul(decay, el.smooth(0.105, el.tapIn({ name: `${name}:fdn${i}` }))));
  });

  const mix = H8.map(function (row: any) {
    return el.add(
      ...row.map(function (col: any, j: number) {
        return el.mul(col * scale, dels[j]);
      }),
  ...
  return mix.map(function (mm: any, i: number) {
    const modulate = (x: any, rate: any, amt: any) => el.add(x, el.mul(amt, el.cycle(rate)));
    ...
    const delaySize = el.mul(el.add(1.0, el.mul(3, size)), ms2samps((i + 1) * 17));
    ...
    return el.tapOut({ name: `${name}:fdn${i}` }, el.delay({ size: ms2samps(750) }, readPos, 0, mm));
  });
}
...
export function reverb(props: ReverbParams, xl: any, xr: any) {
  ...
  const eight = [...four, ...four.map((x: any) => el.mul(-1, x))];
  ...
  // Wet dry mixing
  const outL = el.select(mix, yl, xl);
  const outR = el.select(mix, yr, xr);
