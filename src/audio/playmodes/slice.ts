...
import { InstrumentData, MAX_16BIT } from 'tracker-lib';
import AudioUtils from '@/audio/utils/audioutils.ts';

let inc = 0;

export function slice(path: string, instrument: InstrumentData, pitch: ElemNode, gate: ElemNode) {
  const id = `slice-${inc}`;
  const sliceIndex = instrument.selectedSlice;
  ...
  const sliceIdx = AudioUtils.clamp(sliceIndex, 0, numSlices - 1);
  ...
  const posFrames = el.add(sliceStart, el.mul(elapsed, pitch));
